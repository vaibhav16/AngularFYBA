...
import{IndividualGame} from './../../models/coachIndividualGame.model';
import { ScoreSheetImages } from '../../models/coachReport.model';
import { DeletedScoreSheetImages } from '../../models/coachReport.model';
import { APIGamePost, DeleteIncidentReport } from '../../models/coachReport.model';
...
@Component({
  ...
  styleUrls: ['./coach-gamelist-form.component.css']
})
export class CoachGamelistFormComponent implements OnInit {
  ...
  showIncident(incidentIndex) {
    ...
    this.bsModalRef.content.saveStatus.subscribe(($e) => {
      ...
      for (var i = 0; i < this.coachService.ModifiedIncidents.length; ++i) {
        if (this.coachService.ModifiedIncidents[i].GameId == parseInt(this.gameData.GameId)) {
          this.gameData.IncidentReports[
            incidentIndex
          ].IncidentId = this.coachService.ModifiedIncidents[i].IncidentId;

          this.gameData.IncidentReports[
            incidentIndex
          ].IncidentType = this.coachService.ModifiedIncidents[i].IncidentType;

          this.gameData.IncidentReports[
            incidentIndex
          ].IncidentValue = this.coachService.ModifiedIncidents[i].IncidentValue;

          this.gameData.IncidentReports[
            incidentIndex
          ].Notes = this.coachService.ModifiedIncidents[i].Notes;
        }
      }
    });
  ...
  deleteTempIncident(newIncidentIndex) {
    ...
    for (var i = 0; i < this.coachService.IncidentReports.length; ++i) {
      if (
        this.coachService.IncidentReports[i].IncidentType ==
        this.coachService.NewIncidents[newIncidentIndex].IncidentType
      ) {
        this.coachService.IncidentReports.splice(i, 1);
        this.coachService.NewIncidents.splice(newIncidentIndex, 1);
  ...
  showTempIncident(newIncidentIndex, gameIndex) {
    const initialState = {
      ...
      gameId: this.gameData.GameId,
      incident: this.coachService.IncidentReports[newIncidentIndex],
      allIncidentTypes: this.gameData['IncidentTypes'],
      allDependentDropdowns: this.gameData['IncidentSubDropDown'],
  ...
  async prepareFinalData() {
    ...
    this.APIGamePost.IncidentReports = this.coachService.IncidentReports;
  ...
  showModal() {
    this.coachService.initialJson = JSON.stringify(this.reportGameData);

    const initialState = {
      status: this.coachService.postReportStatus,
      popupTitle: this.coachService.postReportTitle,
      popupMsg: this.coachService.postReportMsg
    };
